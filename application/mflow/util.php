 ...
 *******************************/
    ...
    /*
     * Checks whether the specified IPv4 address belongs to the specified IP address range (net).
     * Parameters:
    ...
    function nfcapd_files_exist ($profile_data_dir, $source, $date, $hours, $minutes) {
        ...
        if ($_SESSION['MFLOW']['nfsen_profile_type'] === "real") {
            $profile = $_SESSION['MFLOW']['nfsen_profile'];
            $source = $source;
        } else {
            $profile = "live";
            $source = "*";
